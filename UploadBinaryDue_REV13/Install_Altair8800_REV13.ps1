...
function Find-Bossac {
    ...
    # FIRST choice: portable bossac.exe in this same folder.
    $candidates += (Join-Path $ScriptDir "bossac.exe")

    # Arduino IDE 2.x board-package location.
    if ($env:LOCALAPPDATA) {
    ...
    throw "bossac.exe was not found. Copy bossac.exe into this folder beside upload.bat, or install Arduino IDE + Arduino SAM Boards."
}
...
try {
    ...
    $bossacArgs = @("--port=$Port", "-U", "false", "-e", "-w", "-v", "-b", $binPath, "-R")
    Write-Host "Running: `"$bossacPath`" $($bossacArgs -join ' ')"

    & $bossacPath @bossacArgs
    $exit = $LASTEXITCODE
